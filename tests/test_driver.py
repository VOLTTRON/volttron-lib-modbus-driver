...
class TestModbusDriver:
    ...
    def get_point(self, publish_agent, point_name):
        """
        ...
        """
        return publish_agent.vip.rpc.call(PLATFORM_DRIVER, 'get_point', 'modbus', point_name).get(timeout=10)

    def set_point(self, publish_agent, point_name, point_value):
        """
        ...
        @return: The returned value from the RPC call.
        """
        return publish_agent.vip.rpc.call(PLATFORM_DRIVER, 'set_point', 'modbus', point_name, point_value).get(timeout=10)

    def scrape_all(self, publish_agent):
        """
        Issue a get_point RPC call for the named point and return the result.
        ...
        @return: The returned value from the RPC call.
        """
        return publish_agent.vip.rpc.call(PLATFORM_DRIVER, 'scrape_all', 'modbus').get(timeout=10)

    def test_default_values(self, publish_agent):
        """
        By default server setting, all registers values are 0
        """
        default_values = self.scrape_all(publish_agent)
        assert type(default_values) is dict

        for key in default_values.keys():
            assert default_values[key] == 0 or 0.0

    def test_set_point(self, publish_agent):
        for key in registers_dict.keys():
            self.set_point(publish_agent, key, registers_dict[key])
            assert self.get_point(publish_agent, key) == registers_dict[key]
        assert self.scrape_all(publish_agent) == registers_dict
